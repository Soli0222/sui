...
単発予定は `recurrence: "monthly"`、`interval: 1`、`startDate` と `endDate` が同じ日付、`dayOfMonth` がその日付の日にち、`dayOfWeek` が `null` であるスケジュールとして扱う。
対象月に一つだけ発生日を生成し、他の月では発生しない。
営業日シフトで前後の月にずれても、単発として一つのイベントだけを残す。
...
ただし単発予定の場合は、対象月の候補だけを生成する段階で `startDate` と `endDate` で区切り、シフト後のずれは捨てない。
