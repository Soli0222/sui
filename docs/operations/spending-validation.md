...
generated: { by: codex/gpt-5.6-sol, at: 2026-09-11T00:00:00+00:00 }
status: draft
---
...
OpenAI・Anthropicは各社の公式HTTPS originだけに接続する。その他の接続先も、DNS解決結果を含めてloopback・private・link-local・予約済みアドレスを拒否する。環境変数のAPIキーは保存済み設定と完全に一致する接続確認にだけ使用し、リクエストだけで指定された接続先には送信しない。
