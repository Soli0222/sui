...
| GET / POST | `/api/subscriptions/:id/amount-changes` | サブスク価格履歴の一覧と追加。適用開始日は YYYY-MM-DD、金額は通貨の最小単位 |
| PUT / DELETE | `/api/subscriptions/:id/amount-changes/:changeId` | 価格履歴の訂正と削除。履歴 ID は一覧の返却値を使用 |
