...
generated: { by: codex/gpt-6, at: 2026-09-25T12:24:22Z }
---
...
一件を読んで操作する一覧は、画面幅にかかわらず一列のカードで表示する。予定収支、口座、カード登録、サブスク、割り勘、寄附、ローン、支出案件、確定キューがこれに当たる。カードの外枠・空状態は `CardList` にまとめ、金額・期間・操作の意味は各画面が決める。名称と主金額を上段に、補足と操作を下段に置く。名称、口座名、メモは折り返して全文を示し、通貨付き金額や日付は途中で分断しない。

同じ項目を縦に比較・入力する取引、予測イベント、寄与イベント、月別請求、給与、精算履歴、MF明細・取込プレビュー、監査ログは広幅で表を使う。狭幅では同じデータを一件ずつ明細リストにする。`ResponsiveTable` は PC の表と狭幅の `CardList` を切り替え、列の多い一覧には個別の切替幅を設定する。カテゴリ別予算のみ狭幅でも4列の表を維持し、その表の領域内で横スクロールできる。
