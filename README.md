...
銀行口座の残高、固定収支、クレジットカードの引き落とし、ローンの返済スケジュールなどを登録し、今後数か月の口座残高推移をチャートで可視化できます。

## 主な機能

| 機能 | 概要 |
|------|------|
| **ダッシュボード** | 合計残高・最小残高・直近の収支を表示し、残高推移をチャートで描画 |
| **口座管理** | 複数の銀行口座を登録し、残高を管理 |
...
| メソッド | パス | 説明 |
|----------|------|------|
| GET | `/api/dashboard` | ダッシュボードデータ（残高予測・イベント一覧） |
