...
![Version: 0.5.0](https://img.shields.io/badge/Version-0.5.0-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: 2.1.0--beta.3](https://img.shields.io/badge/AppVersion-2.1.0--beta.3-informational?style=flat-square)
...
The default `mcp.oauth.resourceUrl: ""` keeps the existing API-token flow. Create a token in the sui UI and send it as `Authorization: Bearer sui_tok_...`.

For OAuth discovery from ChatGPT, configure the public MCP URL together with the same Auth0 custom-domain issuer used by the UI and the existing user's subject:
...
auth:
  ...
  oidc:
    ...
    allowedSubjects:
      - auth0|existing-user-subject
mcp:
  oauth:
    resourceUrl: https://sui.example.com/mcp
```

The Auth0 API audience must exactly match `mcp.oauth.resourceUrl`. OAuth access tokens are not stored by sui. Revoking consent or a refresh token does not necessarily invalidate an already-issued JWT before its expiry; remove the subject from `allowedSubjects` for an immediate sui-side denial.
